...
void getAverageScore(bowlersNames people[], int acc) {  //This function calculates the average of the scores for each player.
	...
	for (int i = 0; i < acc; i++)
	{
		...
		for (int n:people[i].score)
		{
			avg = avg + n;  //The 4 scores for each player gets stored in the 'avg' variable
		}
...
void PrettyPrintResults(bowlersNames people[], int acc)  //Prints the average along with the names and the scores
{
	...
	for (int i = 0; i < acc; i++)
	{
		...
		std::cout << "Score: ";
		for (int n : people[i].score) //Prints the list of scores in the next line
		{
			std::cout << " " << n;
		}
